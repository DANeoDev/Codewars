...
def sudoku(puzzle):
    ...
    for i in range(steps):
        ...
        for el in bestCandidates:
            if el[2]==1: #guaranteedto exist by rules --> harder sudokus: track the actual options [3,7] instead of just the amount; test an option, trace back if it fails
                if puzzle[el[0]][el[1]]==0:
                    for test in range(1,10):
                        puzzle[el[0]][el[1]]=test
                        if rules(puzzle, el[0],el[1]):
                            break
    return puzzle
...
print(total_rules(puzzle))
print(candidates(puzzle))
print(sudoku(puzzle))